  ...
  <header class="header">
    ...
    <div class="navbar">
      <div class="navbar-left-container">
        <a href="/">
          <img class="navbar-brand-logo" src="../../static/logol.png"/>
        </a>
      </div>
...
<script>
  import './../assets/css/login.css'   // 导入登录页面样式
  ...
  export default {
    data() {
      return {
        ...
        userPwd:'',
        errorTip:false,     // 校验提示
        loginModalFlag:false,            // 关掉登录框
        // nickName:''      // 用了vuex这个data数据就不用了
      }
    },
    computed:{
      ...mapState(['nickName','cartCount'])   // 加'...' 表示进行数组的解构赋值（...称为展开运算符）
      // nickName(){
    ...
    methods: {
      checkLogin(){        // 检查是否登录
        axios.get("/users/checkLogin").then((response)=>{
          ...
          if (res.status == '0') {
            // this.nickName = res.result;  // 保存用用户登录名
      ...
      getCartCount(){     // 查询购物车商品数量
        axios.get("/users/getCartCount").then((response)=>{
          let res = response.data;
          // 调用商品数量初始化函数
          this.$store.commit("initCartCount",res.result) // 把res.result即cartCount提交到vuex统一管理
        })
      }
    },
  }
