        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list">
              <li v-for="item in cartList" :key="item.index">
...
<script>
  ...
  import {currency} from "./../util/currency"
    export default {
      ...
      // },   // 已定义全局过滤器，因此不需要局部过滤器了
      computed:{   // 实时计算的是属性，只不过是函数的写法，data里面就不用在声明了
        checkAllFlag(){    // 是否全选属性
          return this.checkedCount == this.cartList.length;  // 勾选的商品总数=购物车商品列表的商品总数时，返回true代表全选。
        },
        checkedCount(){   // 获取已勾选的商品总数(几种商品已勾选)
          var i = 0;
        ...
        totalPrice(){   // 实时计算总价格 （总价格 += 单个商品单价*单个商品数量）
          var money = 0;
          this.cartList.forEach((item)=>{
            if(item.checked=='1'){  // 表示商品被选中
      ...
      methods:{
        init(){    // 初始化商品数据
          axios.get("/users/cartList").then((response)=>{
            ...
            this.cartList = res.result;   // 对商品列表进行赋值
          })
        },
        ...
        delCart(){  //   确认删除此商品
          axios.post("/users/cartDel",{
            productId:this.delItem.productId // 参数（this.delItem当前删除对象）
          }).then((response)=>{
            let res = response.data;
            if (res.status == '0'){
        ...
        editCart(flag,item){    // 商品加减和勾选(item表示当前选中的商品对象)
          if (flag=='add'){       // 添加商品数量
            item.productNum++;    // 商品数量增加
          }else if (flag=='minu'){      // 减少商品数量
            if (item.productNum<=1){   // 保证商品数量至少为1
              return;
            }
            item.productNum--;  // 商品数量减少
          }else {              // 商品控制选中
          ...
          axios.post("users/cartEdit",{
            ...
            // 右上角购物车数量更新
            let num = 0;
            if (flag == 'add'){   // 点击加号，添加数量
                num = 1;
            }else if (flag == 'minu'){  // 点击减号，减少数量
                num = -1;
            }
        ...
        checkOut(){      // 结账
          if (this.checkedCount>0){    // 已勾选的商品总数>0时才可以跳转到地址列表页面
            this.$router.push(
              {path:"/address"}
            )
          }
