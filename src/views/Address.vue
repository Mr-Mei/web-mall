      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>             <!-- 动态绑定class，定义选中索引和循环索引相同，即可实现切换功能 -->
...
<script>
  ...
  export default {
    ...
    computed:{
      addressListFilter(){
        return this.addressList.slice(0,this.limit);  // slice（截取，返回全新数组，不改变原数组） 实现动态控制显示地址列表条数
      }
    },
    methods:{
      init() {    // 渲染地址列表
      ...
      expand(){
        if (this.limit==3){     // 点击more更多
          this.limit = this.addressList.length;  // 把地址列表长度赋予this.limit
        }else {
          this.limit = 3;   // 展开后，再点击就恢复到原来的3条数据
        }
      },
      setDefault(addressId){       // 设置默认地址，需要接收变量addressId
      ...
      delAddressConfirm(addressId){    // 点击删除图标，模态框出现
        if (this.addressList.length>1){  // 地址列表数据大于1条可以删除
            this.isMdShow = true;
      ...
      delAddress(){
        axios.post("/users/delAddress",{
          ...
          if (res.status == '0') {
            console.log("del suc");
            this.isMdShow = false;       // 删除成功后关闭模态框
            this.init();                 // 重新渲染地址列表
          }
        })
      },
