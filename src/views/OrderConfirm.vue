            ...
            <div class="item-list-wrap confirm-item-list-wrap">
              <div class="cart-item order-item">
                ...
                <ul class="cart-item-list">
                  ...
                  可使用template标签进行包裹(template为html5的新标签，无特殊含义) 注意点：key值写在包裹的元素中 -->
                  <template v-for="(item,index) in cartList">
                    <li v-if="item.checked=='1'" :key="index">
              ...
              <div class="price-count">
                <ul>
                  <li>
                    ...
                    <span>{{subTotal|currency('$')}}</span> <!-- {{}}前面有$符号，currency()就不需要传$变量了 -->
                  </li>
                  <li>
                    <span>Shipping:</span>
...
<script>
  ...
  export default {
    ...
    methods:{
      init(){    // 初始化列表
        axios.get("/users/cartList").then((response)=>{   // 订单确认列表不需要再写接口，直接用购物车列表的接口，渲染页面时选取选中的商品作为订单确认的商品
          ...
          this.cartList.forEach((item)=>{    // 遍历购物车商品，获取选中商品的总金额
            if (item.checked=='1'){
              this.subTotal += item.salePrice*item.productNum;  // 所有商品总金额
            }
          })
      ...
      payMent(){     // 点击支付
                     // 从路由那里获取到订单地址的id。(this.$route 全局路由对象)
        ...
        axios.post("/users/payMent",{
          ...
          if (res.status=='0') {
            ...
            // 路由跳转到订单成功页面。(this.$router 获取路由参数,push()方法进行跳转)
            this.$router.push({
